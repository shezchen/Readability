...
namespace UI.Page
{
    ...
    public class LanguagePage : BasePage
    {
        ...
        private void Awake()
        {
            ...
            _uiBinder.Get<UnityEngine.UI.Button>("Button_Chinese").OnClickAsObservable().Subscribe(async(_) =>
            {
                _eventBus.Publish(new LanguageConfirmEvent(GameLanguageType.Chinese));
                await Hide();
                _uiManager.ShowMainScenePage().Forget();
            }).AddTo(this);
            _uiBinder.Get<UnityEngine.UI.Button>("Button_English").OnClickAsObservable().Subscribe(async(_) =>
            {
                _eventBus.Publish(new LanguageConfirmEvent(GameLanguageType.English));
                await Hide();
                _uiManager.ShowMainScenePage().Forget();
            }).AddTo(this);
            _uiBinder.Get<UnityEngine.UI.Button>("Button_Japanese").OnClickAsObservable().Subscribe(async(_) =>
            {
