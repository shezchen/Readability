...
namespace UI.Page
{
    ...
    public class MainScenePage : BasePage
    {
        ...
        [BoxGroup("主页面"),Header("默认选中按钮"), SerializeField] private UnityEngine.UI.Button defaultSelectedButton;
        ...
        private void Awake()
        {
            ...
            {
                if (page.PageType == typeof(SettingsPage) || page.PageType == typeof(StartGamePage))
                {
                    Debug.Log("MainScenePage收到其他Page隐藏事件，显示自己");
            ...
            _eventBus.Receive<PageShow>().Subscribe((page)=>
            {
                if (page.PageType == typeof(SettingsPage) || page.PageType == typeof(StartGamePage))
                {
                    Debug.Log("MainScenePage收到其他Page显示事件，隐藏自己");
                    gameObject.SetActive(false);
                }
            }).AddTo(this);
            
            _uiBinder = GetComponent<UIBinder>();
            _uiBinder.Get<UnityEngine.UI.Button>("Button_Start").OnClickAsObservable().Subscribe(async (_) =>
            {
                _audioService.PlaySfxAsync(AudioClipName.SFX.ClickSound);
                await _uiManager.ShowStartGamePage();
            }).AddTo(this);
            _uiBinder.Get<UnityEngine.UI.Button>("Button_Settings").OnClickAsObservable().Subscribe(async (_) =>
            {
                _audioService.PlaySfxAsync(AudioClipName.SFX.ClickSound);
                await _uiManager.ShowSettingsPage();
            }).AddTo(this);
            _uiBinder.Get<UnityEngine.UI.Button>("Button_Quit").OnClickAsObservable().Subscribe(async (_) =>
            {
                _audioService.PlaySfxAsync(AudioClipName.SFX.ClickSound);
                Application.Quit();
            }).AddTo(this);
